(* OASIS_START *)
(* DO NOT EDIT (digest: 6ddd6f23cd53d5bc1209f35c6905c58c) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("srs", ["lib"])];
     lib_c = [("srs", "lib", [])];
     flags =
       [
          (["oasis_library_srs_ccopt"; "compile"],
            [
               (OASISExpr.EBool true,
                 S [A "-ccopt"; A "-Wall"; A "-ccopt"; A "-Werror"])
            ]);
          (["oasis_library_srs_cclib"; "link"],
            [(OASISExpr.EBool true, S [A "-cclib"; A "-lsrs2"])]);
          (["oasis_library_srs_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-lsrs2"])])
       ];
     includes = [];
     }
  ;;
